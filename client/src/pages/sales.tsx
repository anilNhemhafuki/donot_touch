...
export default function Sales() {
    ...
    const updatedItems = saleForm.items.map((item, i) => {
      if (i === index) {
        if (field === 'productId') {
          ...
          return { ...item, productId: value, unitPrice: product?.price || '0' };
        }
  ...
  const totalSales = sales.reduce((sum: number, sale: Sale) => sum + parseFloat(sale.totalAmount), 0);
    ...
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              Rs. {sales
                .filter((sale: Sale) => new Date(sale.createdAt).toDateString() === new Date().toDateString())
                .reduce((sum: number, sale: Sale) => sum + parseFloat(sale.totalAmount), 0)
                .toLocaleString()}
            </div>
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              Rs. {sales.length > 0 ? (totalSales / sales.length).toLocaleString() : '0'}
            </div>
          </CardContent>
        </Card>
      </div>
        ...
        <CardContent>
              ...
              <div key={sale.id} className="border rounded-lg p-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="text-right">
                    <div className="font-bold">Rs. {parseFloat(sale.totalAmount).toLocaleString()}</div>
          ...
          {filteredSales.length === 0 && (
            <div className="text-center py-8">
              ...
              <p className="text-muted-foreground">
                {searchTerm || statusFilter !== 'all' ? 'Try adjusting your filters' : 'Start by recording your first sale'}
              </p>
            </div>
          )}
        </CardContent>
